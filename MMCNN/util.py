...
def get_dataloader(Input_pickle):
...
def run(input_pickle, lr, epochs, early_stop, seed):
    setup_seed(seed)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    train_loader, test_loader = get_dataloader(input_pickle)
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_loss, val_acc = val(model, device, test_loader, epoch)
        ...
        if best_acc < val_acc:
            ...
            if plateau_period >= early_stop:
                ...
                accuracy, F1, maxtrix = test(model, device, test_loader)
                torch.save(model, 'best.pth')
